...
const Standard_nozzle = (
  length_nozzle = 0.1,
  length_cone = 0.05,
  bore_out_diameter = 0.18,
  bore_in_diameter = 0.12,
  diam_out_cone_bot = 0.12,
  flange_diameter = 0.235,
  raised_f_diameter = 0.185,
  raised_f_thickness = 0.0006,
  flange_thicknes = 0.0144,
  hole_no = 16,
  bolt_diameter = 0.2125,
  bolt_h_size_diam = 0.0112,
  material2 = material1) => {
  material = material2;
  ...
  console.log("position length of cone",length_of_cone,length_of_nozzle)
  console.log("position of cone",position_of_cone)
  length_of_cone===0?mesh_ind2=null:mesh_ind2 = create_component(radius_outer_cone_top, 
    radius_outer_cone_bottom, 
    radius_inner_cone_top,
    radius_inner_cone_bottom, 
    0.05, position_of_cone, 
    0, 0, 0, 0, 3.14 / 2);
  let position_of_second_cylinder = position_of_cone + length_of_cone / 2;
  console.log("position of cylinder",position_of_second_cylinder)
  ...
  //group.add( mesh_ind1 );
  group.add(mesh_ind1);
  length_of_cone===0?console.log():group.add(mesh_ind2);
  group.add(mesh_ind3);
  group.add(mesh_ind4);
...
const create_component = (outer_upper_rad, outer_lower_rad, inner_upper_rad, inner_lower_rad, length, translateX, translateY, translateZ, rotateX, rotateY, rotateZ) => {
  //sphere
  console.log("cone outer ",outer_upper_rad,outer_lower_rad,"length",length,"inner",inner_upper_rad,inner_lower_rad);
